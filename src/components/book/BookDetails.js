...
function BookDetails() {
  ...
  const setFavorite = async () => {
    setIsFavorite(true);
    await axios.post(API_PATHS.setFavorite, bookId, config)
      .then(() => {
        fetchBook()
        fetchIsRated()
        fetchComment()
  ...
  const fetUserProfile = async () => {
    ...
    setFavoriteBooks(data['favorite-books']);
    data['favorite-books'].forEach(favoriteItem => {
  ...
  return (
    <>{favoriteBooks.length > 0 ? <Row className={styles.container}>
        ...
        <Row style={{ paddingTop: '10px' }}>
          ...
          <Col span={14}>
           ...
           <p>
            ...
            { isFavorited ? <Button onClick={setFavorite} shape="circle" icon={!isFavorite ? <HeartOutlined /> : <HeartFilled />} /> : ''} 
